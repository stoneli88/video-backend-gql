...
async function createOrUpdateVideo(parent, args, context, info) {
  ...
  const { id, name, description, category, isEncoded, fileId, owner } = args;
  ...
  if (path) {
    needUpdateParams["file"] = { connect: { id: fileId } };
  }

  needUpdateParams["owner"] = { connect: { id: getUserId(context) } };

  if (id) {
    const video = await context.db.query.video(
      { where: { id: args.id } },
      `{ id name }`
    );
    if (!video) {
    ...
    createOrUpdatedVideo = await context.db.mutation.createVideo(
      { data: needUpdateParams },
      `{ id }`
    );
  }
